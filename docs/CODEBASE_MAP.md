...
Site institucional de página única da DETERA. Next.js 16 (App Router + Turbopack), React 19, TypeScript estrito, Tailwind CSS v4. Tudo é pré-renderizado estático; não existe backend, rota de API nem banco. O formulário de contato monta a mensagem e abre o WhatsApp — nada sai do navegador para um servidor nosso.
...
|---|---|---|---|
| `hero.tsx` | Nome caindo, slogan, CTAs, trilha das frentes, céu, auras, jogo na margem | `pilares`, `site.slogan` | 2097 |
...
| `personalidade.tsx` | O slogan explicado: genérico × específico | `personalidade` | 751 |
...
| `.aura` | Brilho radial à deriva, parametrizado por `--aura-*` |
| `.estrela`, `.ceu-deriva`, `.cadente`, `.cometa` | O céu |
...
| `sections/case-preview.tsx` | Fallback quando a imagem ou o iframe falha |
...
- **Criar uma animação** → keyframe e classe em `globals.css`. Movimento reduzido já é global; se usar `view()`, envolva em `@supports` e adicione ao bloco `@media print`.
