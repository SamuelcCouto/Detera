...
/**
 * Rótulo de etapa do case, empilhado (não em coluna ao lado do texto): o
 * card agora divide a tela com outro, e uma coluna de rótulo fixa não sobra
 * largura nenhuma numa faixa dessa largura.
 */
...
export function Cases() {
  return (
    ...
    <Section id="projetos" tone="vazio" aria-labelledby="cases-titulo">
      <Container>
          ...
          {cases.map((caso) => (
            <article key={caso.id} data-surgir className="bloco flex flex-col">
              <PreviaCase
                ...
                imagemUrl={caso.imagemUrl}
                imagemAlt={caso.imagemAlt}
              />
        ...
        {projetosEmConstrucao.length > 0 ? (
          <div className="mt-16 border-t border-borda pt-12">
